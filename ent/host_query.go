...
type HostQuery struct {
	...
	withFileTickets *FileTicketQuery
...
func (hq *HostQuery) QueryAdmins() *AdminQuery {
...
// QueryFileTickets chains the current query on the "file_tickets" edge.
func (hq *HostQuery) QueryFileTickets() *FileTicketQuery {
	query := &FileTicketQuery{config: hq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := hq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (hq *HostQuery) Clone() *HostQuery {
	...
	return &HostQuery{
		...
		withFileTickets: hq.withFileTickets.Clone(),
...
func (hq *HostQuery) WithAdmins(opts ...func(*AdminQuery)) *HostQuery {
	query := &AdminQuery{config: hq.config}
...
// WithFileTickets tells the query-builder to eager-load the nodes that are connected to
// the "file_tickets" edge. The optional arguments are used to configure the query builder of the edge.
func (hq *HostQuery) WithFileTickets(opts ...func(*FileTicketQuery)) *HostQuery {
	query := &FileTicketQuery{config: hq.config}
	for _, opt := range opts {
		opt(query)
	}
	hq.withFileTickets = query
	return hq
}
...
func (hq *HostQuery) sqlAll(ctx context.Context) ([]*Host, error) {
	...
	if query := hq.withAdmins; query != nil {
		...
		query.Where(predicate.Admin(func(s *sql.Selector) {
			s.Where(sql.InValues(host.AdminsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.host_admins
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "host_admins" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "host_admins" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Admins = append(node.Edges.Admins, n)
		}
	}

	if query := hq.withFileTickets; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Host)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.FileTickets = []*FileTicket{}
		}
		query.withFKs = true
		query.Where(predicate.FileTicket(func(s *sql.Selector) {
			s.Where(sql.InValues(host.FileTicketsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.host_file_tickets
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "host_file_tickets" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "host_file_tickets" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.FileTickets = append(node.Edges.FileTickets, n)
		}
	}

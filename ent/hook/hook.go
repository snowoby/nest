...
// The FileTicketFunc type is an adapter to allow the use of ordinary
// function as FileTicket mutator.
type FileTicketFunc func(context.Context, *ent.FileTicketMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f FileTicketFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.FileTicketMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.FileTicketMutation", m)
	}
	return f(ctx, mv)
}

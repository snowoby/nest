...
func (aq *AdminQuery) QueryHost() *HostQuery {
...
func (aq *AdminQuery) WithHost(opts ...func(*HostQuery)) *AdminQuery {
	query := &HostQuery{config: aq.config}
...
func (aq *AdminQuery) sqlAll(ctx context.Context) ([]*Admin, error) {
	...
	if query := aq.withHost; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Admin)
		for i := range nodes {
			if fk := nodes[i].host_admins; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "host_admins" returned %v`, n.ID)
			}
			for i := range nodes {

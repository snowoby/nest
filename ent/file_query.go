...
func (fq *FileQuery) QueryFileTickets() *FileTicketQuery {
	query := &FileTicketQuery{config: fq.config}
...
func (fq *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withFileTickets: fq.withFileTickets.Clone(),
...
func (fq *FileQuery) WithFileTickets(opts ...func(*FileTicketQuery)) *FileQuery {
	query := &FileTicketQuery{config: fq.config}
	for _, opt := range opts {
		opt(query)
	}
	fq.withFileTickets = query
	return fq
}
...
func (fq *FileQuery) sqlAll(ctx context.Context) ([]*File, error) {
	var (
		...
		_spec       = fq.querySpec()
		loadedTypes = [1]bool{
			fq.withFileTickets != nil,
		}
	)
	if fq.withFileTickets != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := fq.withFileTickets; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*File)
		for i := range nodes {
			if fk := nodes[i].file_ticket_files; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(fileticket.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "file_ticket_files" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.FileTickets = n
			}
		}
	}
